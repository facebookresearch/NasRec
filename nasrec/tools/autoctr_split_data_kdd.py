...
def main(args):  # noqa
    ...
    training_file = os.path.join(args.data_dir, "training.txt")
    user_file = os.path.join(args.data_dir, "userid_profile.txt")
    ...
    with open(training_file, "r") as fp:
        while True:
            ...
            try:
                item = user_dict[splitted_lines[-1]]
            except Exception:
                item = ["0", "0"]
            # del spliited_lines[-1]
            splitted_lines += item
            # Re-order 14 features.
            reorder_id = [0, 1, 5, 6, 2, 3, 4, 7, 8, 9, 10, 11, 12, 13]
            splitted_lines_reordered = [splitted_lines[x] for x in reorder_id]
            all_labels.append(label)
            num_counts += 1
            if num_counts % 1000000 == 0:
                print("Processed {} examples!".format(num_counts))
            all_lines.append(splitted_lines_reordered[INT_FEATS+1:])
    ...
    assert all_lines.shape[-1] == CAT_FEATS, ValueError("{} v.s. {}".format(
        all_lines.shape[-1], CAT_FEATS))
    ...
    with open(training_file, "r") as fp:
        ...
        while True:
            ...
            reorder_id = [0, 1, 5, 6, 2, 3, 4, 7, 8, 9, 10, 11, 12, 13]
            splitted_lines_reordered = [splitted_lines[x] for x in reorder_id]
            splitted_lines_reordered[0] = "1" if int(splitted_lines_reordered[0]) > 0 else "0"
            for idx in range(INT_FEATS+1, len(splitted_lines_reordered)):
                try:
                    literal = "{:08x}".format(int(splitted_lines_reordered[idx], 10) % 2147483647)
                except Exception:
                    literal = "{:08x}".format(int(splitted_lines_reordered[idx], 16) % 2147483647)
                splitted_lines_reordered[idx] = literal
            # Push label to binary.
            line = "\t".join(splitted_lines_reordered) + "\n"
